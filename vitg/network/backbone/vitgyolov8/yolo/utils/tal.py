...
def select_candidates_in_gts(xy_centers, gt_bboxes, eps=1e-9, roll_out=False):
    ...
    if roll_out:
        ...
        for b in range(bs):
            ...
            bbox_deltas[b] = (
                torch.cat((xy_centers[None] - lt, rb - xy_centers[None]), dim=2)
                .view(n_boxes, n_anchors, -1)
                .amin(2)
                .gt_(eps)
            )
        return bbox_deltas
...
class TaskAlignedAssigner(nn.Module):
    def __init__(
        self, topk=13, num_classes=80, alpha=1.0, beta=6.0, eps=1e-9, roll_out_thr=0
    ):
        super().__init__()
        self.topk = topk
    ...
    def forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        self.roll_out = (
            self.n_max_boxes > self.roll_out_thr if self.roll_out_thr else False
        )
    ...
    def get_pos_mask(
        ...
        mask_in_gts = select_candidates_in_gts(
            anc_points, gt_bboxes, roll_out=self.roll_out
        )
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes):
        if self.roll_out:
            align_metric = torch.empty(
                (self.bs, self.n_max_boxes, pd_scores.shape[1]), device=pd_scores.device
            )
            overlaps = torch.empty(
                (self.bs, self.n_max_boxes, pd_scores.shape[1]), device=pd_scores.device
            )
            ...
            for b in range(self.bs):
                ...
                overlaps[b] = (
                    bbox_iou(
                        gt_bboxes[b].unsqueeze(1),
                        pd_bboxes[b].unsqueeze(0),
                        xywh=False,
                        CIoU=True,
                    )
                    .squeeze(2)
                ...
                align_metric[b] = bbox_scores.pow(self.alpha) * overlaps[b].pow(
                    self.beta
                )
        else:
            ind = torch.zeros(
    ...
    def select_topk_candidates(self, metrics, largest=True, topk_mask=None):
        ...
        if self.roll_out:
            is_in_topk = torch.empty(
                metrics.shape, dtype=torch.long, device=metrics.device
            )
