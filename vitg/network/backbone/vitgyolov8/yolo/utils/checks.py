...
def check_font(font="Arial.ttf"):
    ...
    url = f"https://vitg.vitgyolov8.com/assets/{name}"
...
def check_requirements(
    ...
    if s and install and AUTOINSTALL:  # check environment variable
        LOGGER.info(
            f"{prefix} YOLOv8 requirement{'s' * (n > 1)} {s}not found, attempting AutoUpdate..."
        )
        try:
...
def check_yolov5u_filename(file: str):
    ...
    if "yolov3" in file or "yolov5" in file and "u" not in file:
        original_file = file
        file = re.sub(
            r"(.*yolov5([nsmlx]))\.", "\\1u.", file
        )  # i.e. yolov5n.pt -> yolov5nu.pt
        file = re.sub(
            r"(.*yolov3(|-tiny|-spp))\.", "\\1u.", file
        )  # i.e. yolov3-spp.pt -> yolov3-sppu.pt
        if file != original_file:
            LOGGER.info(
                f"PRO TIP 💡 Replace 'model={original_file}' with new 'model={file}'.\nYOLOv5 'u' models are "
                f"trained with https://github.com/vitg.vitgyolov8/vitg.vitgyolov8 and feature improved performance vs "
                f"standard YOLOv5 models trained with https://github.com/vitg.vitgyolov8/yolov5.\n"
            )
    return file
...
def check_yolo(verbose=True):
    from vitg.network.backbone.vitgyolov8.yolo.utils.torch_utils import select_device
