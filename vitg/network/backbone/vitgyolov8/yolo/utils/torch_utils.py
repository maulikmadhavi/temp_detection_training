...
def DDP_model(model):
    ...
    assert not check_version(torch.__version__, "1.12.0", pinned=True), (
        ...
        "Please upgrade or downgrade torch to use DDP. See https://github.com/vitg.vitgyolov8/yolov5/issues/8395"
    )
    if TORCH_1_11:
        return DDP(
...
def select_device(device="", batch=0, newline=False):
    ...
    s = f"vitg.vitgyolov8 YOLOv{__version__} 🚀 Python-{platform.python_version()} torch-{torch.__version__} "
...
def model_info(model, verbose=False, imgsz=640):
    ...
    m = (
        Path(
            getattr(model, "yaml_file", "") or model.yaml.get("yaml_file", "")
        ).stem.replace("yolo", "YOLO")
        or "Model"
    )
    LOGGER.info(
        f"{m} summary{fused}: {len(list(model.modules()))} layers, {n_p} parameters, {n_g} gradients{fs}"
    )
...
def init_seeds(seed=0, deterministic=False):
    ...
    # torch.backends.cudnn.benchmark = True  # AutoBatch problem https://github.com/vitg.vitgyolov8/yolov5/issues/9287
    if (
        deterministic and TORCH_1_12
    ):  # https://github.com/vitg.vitgyolov8/yolov5/pull/8213
...
def strip_optimizer(f="best.pt", s=""):
    ...
    Usage:
        from vitg.network.backbone.vitgyolov8.yolo.utils.torch_utils import strip_optimizer
        from pathlib import Path
