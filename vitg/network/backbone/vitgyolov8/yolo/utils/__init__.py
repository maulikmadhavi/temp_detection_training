...
from vitg.network.backbone.vitgyolov8 import __version__
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date vitg.vitgyolov8
...
def get_latest_pypi_version(package_name="vitg.vitgyolov8"):
    """
...
def get_user_config_dir(sub_dir="vitg.vitgyolov8"):
    """
...
USER_CONFIG_DIR = get_user_config_dir()  # vitg.vitgyolov8 settings dir


def emojis(string=""):
    # Return platform-dependent emoji-safe version of string
    return (
        string.encode().decode("ascii", "ignore")
...
def set_sentry():
    ...
    if (
        ...
        and not is_pytest_running()
        and not is_github_actions_ci()
        and (
            (is_pip_package() and not is_git_dir())
            or (
                get_git_origin_url()
                == "https://github.com/vitg.vitgyolov8/vitg.vitgyolov8.git"
                and get_git_branch() == "main"
            )
        )
    ):
...
def get_settings(file=USER_CONFIG_DIR / "settings.yaml", version="0.0.2"):
    ...
    from vitg.network.backbone.vitgyolov8.yolo.utils.checks import check_version
    ...
    defaults = {
        ...
        ).hexdigest(),  # anonymized uuid hash
        "settings_version": version,
    }  # vitg.vitgyolov8 settings version
    ...
    with torch_distributed_zero_first(RANK):
        ...
        if not correct:
            LOGGER.warning(
                "WARNING ⚠️ vitg.vitgyolov8 settings reset to defaults. This is normal and may be due to a "
                "recent vitg.vitgyolov8 package update, but may have overwritten previous settings. "
...
PREFIX = colorstr("vitg.vitgyolov8: ")
