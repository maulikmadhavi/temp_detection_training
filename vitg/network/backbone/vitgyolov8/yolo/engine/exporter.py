...
from vitg.network.backbone.vitgyolov8.nn.autobackend import check_class_names
from vitg.network.backbone.vitgyolov8.nn.modules import Detect, Segment
from vitg.network.backbone.vitgyolov8.nn.tasks import DetectionModel, SegmentationModel
from vitg.network.backbone.vitgyolov8.yolo.cfg import get_cfg
from vitg.network.backbone.vitgyolov8.yolo.data.dataloaders.stream_loaders import (
    LoadImages,
)
from vitg.network.backbone.vitgyolov8.yolo.data.utils import (
    IMAGENET_MEAN,
    IMAGENET_STD,
    check_det_dataset,
)
...
from vitg.network.backbone.vitgyolov8.yolo.utils.checks import (
    check_imgsz,
    check_requirements,
    check_version,
    check_yaml,
)
from vitg.network.backbone.vitgyolov8.yolo.utils.files import file_size
from vitg.network.backbone.vitgyolov8.yolo.utils.ops import Profile
...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        if self.args.half:
            ...
            ), "--half not compatible with --dynamic, i.e. use either --half or --dynamic"
        ...
        self.output_shape = (
            ...
            else tuple(tuple(x.shape) for x in y)
        )
        self.pretty_name = self.file.stem.replace("yolo", "YOLO")
        self.metadata = {
            "description": f"vitg.vitgyolov8 {self.pretty_name} model trained on {self.args.data}",
            "author": "vitg.vitgyolov8",
            "license": "GPL-3.0 https://vitg.vitgyolov8.com/license",
            "version": __version__,
        ...
        if any(f):
            ...
            s = (
                f"WARNING ⚠️ non-PyTorch val requires square images, 'imgsz={self.imgsz}' will not work. Use "
                f"export 'imgsz={max(self.imgsz)}' if val is required."
                if not square
                else ""
            )
    ...
    def _export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        yaml_save(
            Path(f) / self.file.with_suffix(".yaml").name, self.metadata
        )  # add metadata.yaml
        return f, None
    ...
    def _export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        class iOSModel(torch.nn.Module):
            # Wrap an vitg.vitgyolov8 YOLO model for iOS export
    ...
    def _export_saved_model_OLD(
        ...
        tf_model = tf_models.TFModel(
            cfg=self.model.yaml, model=self.model.cpu(), nc=self.model.nc, imgsz=imgsz
        )
        im = tf.zeros((batch_size, *imgsz, ch))  # BHWC order for TensorFlow
        _ = tf_model.predict(
            im, nms, agnostic_nms, topk_per_class, topk_all, iou_thres, conf_thres
        )
        inputs = tf.keras.Input(
            shape=(*imgsz, ch), batch_size=None if self.args.dynamic else batch_size
        )
        outputs = tf_model.predict(
            inputs, nms, agnostic_nms, topk_per_class, topk_all, iou_thres, conf_thres
        )
        ...
        if self.args.keras:
            ...
            tfm.__call__ = tf.function(
                lambda x: frozen_func(x)[:4] if nms else frozen_func(x), [spec]
            )
            tfm.__call__(im)
            tf.saved_model.save(
                tfm,
    ...
    def _export_tflite(
        ...
        if int8:
            ...
            dataset = LoadImages(
                check_det_dataset(check_yaml(data))["train"], imgsz=imgsz, auto=False
            )
            converter.representative_dataset = lambda: representative_dataset_gen(
                dataset, n_images=100
            )
    ...
    def _export_edgetpu(self, prefix=colorstr("Edge TPU:")):
        ...
        f = str(self.file).replace(
            self.file.suffix, "-int8_edgetpu.tflite"
        )  # Edge TPU model
