...
from vitg.network.backbone.vitgyolov8.yolo.utils import LOGGER, ops
...
class Results:
    ...
    def __init__(self, boxes=None, masks=None, probs=None, orig_shape=None) -> None:
        self.boxes = (
            Boxes(boxes, orig_shape) if boxes is not None else None
        )  # native size boxes
        self.masks = (
            Masks(masks, orig_shape) if masks is not None else None
...
class Boxes:
    ...
    def __repr__(self):
        return (
            f"vitg.vitgyolov8 YOLO {self.__class__} masks\n"
...
if __name__ == "__main__":
    # test examples
    results = Results(
        boxes=torch.randn((2, 6)),
        masks=torch.randn((2, 160, 160)),
        orig_shape=[640, 640],
    )
