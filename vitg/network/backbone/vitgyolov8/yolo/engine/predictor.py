...
from vitg.network.backbone.vitgyolov8.yolo.utils.checks import check_imgsz, check_imshow
...
class BasePredictor:
    ...
    def stream_inference(self, source=None, model=None):
        ...
        if self.args.verbose and self.seen:
            ...
            LOGGER.info(
                f"Speed: %.1fms pre-process, %.1fms inference, %.1fms postprocess per image at shape "
                f"{(1, 3, *self.imgsz)}" % t
            )
