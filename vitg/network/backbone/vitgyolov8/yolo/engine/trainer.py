...
from vitg.network.backbone.vitgyolov8.yolo.utils.autobatch import check_train_batch_size
from vitg.network.backbone.vitgyolov8.yolo.utils.checks import (
    check_file,
    check_imgsz,
    print_args,
)
from vitg.network.backbone.vitgyolov8.yolo.utils.dist import (
    ddp_cleanup,
    generate_ddp_command,
)
from vitg.network.backbone.vitgyolov8.yolo.utils.files import (
    get_latest_run,
    increment_path,
)
...
# from vitg.network.backbone.vitgyolov8.yolo.v8.detect.trainv2 import DetectionTrainer
    ...
    def train(self):
        ...
        if world_size > 1 and "LOCAL_RANK" not in os.environ:
            cmd, file = generate_ddp_command(
                world_size, self
            )  # security vulnerability in Snyk scans
            try:
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    # print("bach image shape")
                    # print(batch["img"].shape)
                    preds = self.model(batch["img"])
                    # print("preds out shape")
                    # print(len(preds))
                    # for idxitem in preds:
                    #    print(idxitem.shape)

                    # self.loss, self.loss_items = DetectionTrainer.criterion(preds, batch)
