...
from vitg.network.backbone.vitgyolov8.yolo.data.utils import check_det_dataset
...
from vitg.network.backbone.vitgyolov8.yolo.utils.downloads import unzip_file
from vitg.network.backbone.vitgyolov8.yolo.utils.ops import (
    clean_str,
    segments2boxes,
    xyn2xy,
    xywh2xyxy,
...
class LoadImages:
    ...
    def _new_video(self, path):
        ...
        # self.cap.set(cv2.CAP_PROP_ORIENTATION_AUTO, 0)  # disable https://github.com/vitg.vitgyolov8/yolov5/issues/8493
...
):  # from vitg.utils.dataloaders import *; extract_boxes()
...
def autosplit(
    ...
    Usage: from vitg.utils.dataloaders import *; autosplit()
    Arguments
...
class HUBDatasetStats:
    ...
    Usage
        from vitg.utils.dataloaders import HUBDatasetStats
    ...
    def get_json(self, save=False, verbose=False):
        # Return dataset JSON for vitg.vitgyolov8 HUB
        def _round(labels):
    ...
    def process_images(self):
        # Compress images for vitg.vitgyolov8 HUB
        for split in "train", "val", "test":
            if self.data.get(split) is None:
                continue
            dataset = LoadImagesAndLabels(self.data[split])  # load dataset
            ...
            with ThreadPool(NUM_THREADS) as pool:
                for _ in tqdm(
                    pool.imap(self._hub_ops, dataset.im_files), total=total, desc=desc
                ):
                    pass
