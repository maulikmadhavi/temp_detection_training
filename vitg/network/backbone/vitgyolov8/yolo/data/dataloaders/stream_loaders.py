...
from vitg.network.backbone.vitgyolov8.yolo.data.augment import LetterBox
...
from vitg.network.backbone.vitgyolov8.yolo.utils.checks import check_requirements
...
class LoadStreams:
    ...
    def __init__(
        ...
        s = np.stack(
            [LetterBox(imgsz, auto, stride=stride)(image=x).shape for x in self.imgs]
        )
        self.rect = (
    ...
    def __next__(self):
        ...
        if self.transforms:
            ...
            im = np.stack(
                [
                    LetterBox(self.imgsz, self.auto, stride=self.stride)(image=x)
                    for x in im0
                ]
            )
...
class LoadImages:
    ...
    def _new_video(self, path):
        ...
        if hasattr(cv2, "CAP_PROP_ORIENTATION_META"):  # cv2<4.6.0 compatibility
            ...
            # Disable auto-orientation due to known issues in https://github.com/vitg.vitgyolov8/yolov5/issues/8493
...
def autocast_list(source):
    ...
    for im in source:
        if isinstance(im, (str, Path)):  # filename or uri
            ...
            raise TypeError(
                f"type {type(im).__name__} is not a supported vitg.vitgyolov8 prediction source type. \n"
                f"See https://docs.vitg.vitgyolov8.com/predict for supported source types."
            )
