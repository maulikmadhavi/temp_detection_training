...
from vitg.network.backbone.vitgyolov8.yolo.data.utils import IMG_FORMATS, VID_FORMATS
from vitg.network.backbone.vitgyolov8.yolo.utils.checks import check_file
...
def build_dataloader(
    cfg, batch, img_path, stride=32, rect=False, names=None, rank=-1, mode="train"
):
    ...
    loader = (
        DataLoader if cfg.image_weights or cfg.close_mosaic else InfiniteDataLoader
...
def check_source(source):
    ...
    if isinstance(source, (str, int, Path)):  # int for local usb carame
        ...
        raise Exception(
            "Unsupported type encountered! See docs for supported types https://docs.vitg.vitgyolov8.com/predict"
        )
...
def load_inference_source(
    source=None, transforms=None, imgsz=640, vid_stride=1, stride=32, auto=True
):
    """
    TODO: docs
    """
    ...
    source_type = (
        source.source_type if in_memory else SourceTypes(webcam, screenshot, from_img)
    )
    ...
    if in_memory:
        ...
        dataset = LoadScreenshots(
            source, imgsz=imgsz, stride=stride, auto=auto, transforms=transforms
        )
    elif from_img:
