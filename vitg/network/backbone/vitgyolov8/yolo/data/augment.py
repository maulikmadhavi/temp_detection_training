...
class RandomPerspective:
    ...
    def apply_keypoints(self, keypoints, M):
        ...
        new_keypoints[:, :2] = keypoints.reshape(
            n * 17, 2
        )  # num_kpt is hardcoded to 17
        new_keypoints = new_keypoints @ M.T  # transform
        new_keypoints = (new_keypoints[:, :2] / new_keypoints[:, 2:3]).reshape(
            n, 34
        )  # perspective rescale or affine
        ...
        x_kpts[
            np.logical_or.reduce(
                (x_kpts < 0, x_kpts > self.size[0], y_kpts < 0, y_kpts > self.size[1])
            )
        ] = 0
        y_kpts[
            np.logical_or.reduce(
        ...
        if augment:  # Resize and crop
            ...
            if auto_aug:
                ...
                if jitter > 0:
                    color_jitter = (
                        float(jitter),
                    ) * 3  # repeat value for brightness, contrast, saturation, 0 hue
