...
CLI_HELP_MSG = f"""
    Arguments received: {str(['yolo'] + sys.argv[1:])}. Note that vitg.vitgyolov8 'yolo' commands use the following syntax:
    ...
    Docs: https://docs.vitg.vitgyolov8.com/cli
...
def entrypoint(debug=""):
    """
    This function is the vitg.vitgyolov8 package entrypoint, it's responsible for parsing the command line arguments passed
    to the package.
        ...
        if "=" in a:
            try:
                ...
                if k == "cfg":  # custom.yaml passed
                    ...
                    overrides = {
                        k: val for k, val in yaml_load(v).items() if k != "cfg"
                    }
                else:
                    if v.lower() == "none":
                        v = None
    ...
    # Defaults
    task2model = dict(
        detect="yolov8n.pt", segment="yolov8n-seg.pt", classify="yolov8n-cls.pt"
    )
    task2data = dict(
        detect="coco128.yaml", segment="coco128-seg.yaml", classify="mnist160"
    )
    ...
    if task and task != model.task:
        LOGGER.warning(
            f"WARNING ⚠️ 'task={task}' conflicts with {model.task} model {overrides['model']}. "
    ...
    if mode == "predict" and "source" not in overrides:
        overrides["source"] = (
            ...
            else "https://vitg.vitgyolov8.com/images/bus.jpg"
        )
        LOGGER.warning(
        ...
        if "data" not in overrides:
            ...
            LOGGER.warning(
                f"WARNING ⚠️ 'data' is missing. Using {model.task} default 'data={overrides['data']}'."
            )
    elif mode == "export":
