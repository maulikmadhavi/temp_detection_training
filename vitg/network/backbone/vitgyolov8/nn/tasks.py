...
from vitg.network.backbone.vitgyolov8.nn.modules import (
    C1,
    C2,
    C3,
    C3TR,
...
class DetectionModel(BaseModel):
    ...
    def __init__(
        ...
        self.yaml = (
            ...
            else yaml_load(check_yaml(cfg), append_filename=True)
        )  # cfg dict
        ...
        self.model, self.save = parse_model(
            deepcopy(self.yaml), ch=ch, verbose=False
        )  # model, savelist
...
class ClassificationModel(BaseModel):
    ...
    def __init__(
        ...
        self._from_detection_model(
            model, nc, cutoff
        ) if model is not None else self._from_yaml(cfg, ch, nc, verbose)
...
def torch_safe_load(weight):
    ...
    try:
        ...
        if e.name == "omegaconf":  # e.name is missing module name
            LOGGER.warning(
                f"WARNING ⚠️ {weight} requires {e.name}, which is not in vitg.vitgyolov8 requirements."
                f"\nAutoInstall will run now for {e.name} but this feature will be removed in the future."
                f"\nRecommend fixes are to train a new model using updated vitg.vitgyolov8 package or to "
                f"download updated models from https://github.com/vitg.vitgyolov8/assets/releases/tag/v0.0.0"
            )
        if e.name != "models":
...
def guess_model_task(model):
    ...
    raise SyntaxError(
        "YOLO is unable to automatically guess model task. Explicitly define task for your model, "
