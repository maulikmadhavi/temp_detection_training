...
from vitg.network.backbone.vitgyolov8.yolo.utils import LOGGER, ROOT, yaml_load
...
from vitg.network.backbone.vitgyolov8.yolo.utils.downloads import (
    attempt_download_asset,
    is_url,
)
from vitg.network.backbone.vitgyolov8.yolo.utils.ops import xywh2xyxy
...
def check_class_names(names):
    ...
    if isinstance(names[0], str) and names[0].startswith(
        ...
        map = yaml_load(ROOT / "yolo/data/datasets/ImageNet.yaml")[
            "map"
...
class AutoBackend(nn.Module):
    def __init__(
        ...
        """
        MultiBackend class for python inference on various platforms using vitg.vitgyolov8 YOLO.
        ...
        if nn_module:
            ...
            from vitg.network.backbone.vitgyolov8.nn.tasks import attempt_load_weights
            ...
            from vitg.utils.triton import TritonRemoteModel
            model = TritonRemoteModel(url=w)
            nhwc = model.runtime.startswith("tensorflow")
            """
        else:
            from vitvitg.network.backboneg.vitgyolov8.yolo.engine.exporter import (
                EXPORT_FORMATS_TABLE,
            )

            raise TypeError(
                f"model='{w}' is not a supported model format. "
                "See https://docs.vitg.vitgyolov8.com/tasks/detection/#export for help."
                f"\n\n{EXPORT_FORMATS_TABLE}"
            )
        ...
        if "names" not in locals():  # names missing
            names = (
                yaml_load(check_yaml(data))["names"]
                if data
    ...
    def _model_type(p="path/to/model.pt"):
        ...
        from vitg.network.backbone.vitgyolov8.yolo.engine.exporter import export_formats
