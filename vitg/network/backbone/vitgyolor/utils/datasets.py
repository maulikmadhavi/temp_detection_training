...
from vitg.network.backbone.vitgyolor.utils.general import xywh2xyxy, xyxy2xywh
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        templabelpath = "/home/alex/data/coco/dell1_val_data/annotations/yolo_anno_all_val80class_0_79_nocrowd_images/"

        self.img_files = [
            item
            for item in self.img_files
            if os.path.isfile(
                templabelpath
                + item.split("/")[-1].replace(".jpg", ".txt").replace(".png", ".txt")
            )
        ]
        self.img_files = [
            item
