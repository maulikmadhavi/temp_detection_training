...
from vitg.network.backbone.vitgyolor.utils.datasets import letterbox
...
class BottleneckCSPLG(nn.Module):
    # CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(
        self, c1, c2, n=1, shortcut=True, g=3, e=0.25
    ):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.act = nn.SiLU()
        self.m = nn.Sequential(
            *[BottleneckG(g * c_, g * c_, shortcut, g, e=1.0) for _ in range(n)]
        )
...
class BottleneckCSPDNL(nn.Module):
    ...
    def forward(self, x):
        ...
        # [N, C', T,  H, W]
        out_sim = out_sim.view(
            out_sim.size(0), out_sim.size(1), *y.size()[2:]
        ).contiguous()
