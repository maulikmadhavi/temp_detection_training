...
# from vitg.network.backbone.vitgyolov7.utils.google_utils import gsutil_getsize
# from vitg.network.backbone.vitgyolov7.utils.metrics import fitness
from vitg.network.backbone.vitgyolov7.utils.torch_utils import init_torch_seeds
...
def non_max_suppression_kpt(
    ...
    if nc is None:
        nc = (
            prediction.shape[2] - 56 if kpt_label else prediction.shape[2] - 5
        )  # number of classes
    xc = prediction[..., 4] > conf_thres  # candidates
...
def strip_optimizer(
    f="best.pt", s=""
):  # from vitg.utils.general import *; strip_optimizer()
