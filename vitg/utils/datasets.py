...
def create_dataloader_fortest3(
    path,
    imgsz,
    batch_size,
    stride,
    ...
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache.
    with torch_distributed_zero_first(local_rank):
        dataset = LoadImagesAndLabels_test3(
            path,
            imgsz,
            batch_size,
            augment=augment,  # augment images
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        pin_memory=True,
        # collate_fn=None
        collate_fn=LoadImagesAndLabels_test3.collate_fn,
    )
    return dataloader, dataset


def create_dataloader_fortest_recfalse(
    path,
    imgsz,
    batch_size,
    stride,
    ...
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache.
    with torch_distributed_zero_first(local_rank):
        dataset = LoadImagesAndLabels_test_recfalse(
            path,
            imgsz,
            batch_size,
            augment=augment,  # augment images
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        pin_memory=True,
        # collate_fn=None
        collate_fn=LoadImagesAndLabels_test_recfalse.collate_fn,
    )
    return dataloader, dataset
...
class LoadImagesAndLabels_squaretest(Dataset):  # for training/testing
    def __init__(
        self,
        path,
        img_size=640,
...
class LoadImagesAndLabels_test3(Dataset):  # for training/testing
    def __init__(
        self,
        path,
        img_size=640,
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            shape = shape0[0] if self.rect else self.img_size
            img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)
            # print("img sum after letter box")
            # print(img.shape)
            # print(img.sum())
...
class LoadImagesAndLabels_test_recfalse(Dataset):  # for training/testing
    def __init__(
        self,
        path,
        img_size=640,
    ...
    @staticmethod
    def collate_fn(batch):
        img, label, path, shapes, shape_recfalse = zip(*batch)  # transposed
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label, 0), path, shapes, shape_recfalse
