...
from vitg.utils.torch_utils import init_seeds, is_parallel
...
def strip_optimizer(
    f="weights/best.pt", s=""
):  # from vitg.utils.utils import *; strip_optimizer()
...
):  # from vitg.utils.utils import *; coco_only_people()
...
):  # from vitg.utils.utils import *; crop_images_random()
...
def coco_single_class_labels(path="../coco/labels/train2014/", label_class=43):
    # Makes single-class coco datasets. from vitg.utils.utils import *; coco_single_class_labels()
...
def kmean_anchors(
    ...
    Usage:
        from vitg.utils.utils import *; _ = kmean_anchors()
    """
    thr = 1.0 / thr
    ...
    if isinstance(path, str):  # *.yaml file
        ...
        from vitg.utils.datasets import LoadImagesAndLabels
...
def output_to_target_local_convert(output, width, height):
    # in order to converter correct box location with padding
    img_size = max(width, height)
    # Convert model output to target format [batch_id, class_id, x, y, w, h, conf]
    # and conver cordinate back to refer to original width and height(remove change due to padding)
    ...
    for i, o in enumerate(output):
        if o is not None:
            for pred in o:
                ...
                wpad = (img_size - width) / 2
                hpad = (img_size - height) / 2

                xminnew = xminori - wpad
                xmaxnew = xmaxori - wpad
                yminnew = yminori - hpad
                ymaxnew = ymaxori - hpad

                w = (xmaxnew - xminnew) / width
                h = (ymaxnew - yminnew) / height
                x = xminnew / width + w / 2
                y = yminnew / height + h / 2
                conf = pred[4]
                cls = int(pred[5])
                w = w.cpu()
                h = h.cpu()
...
def increment_dir(dir, comment=""):
    ...
    d = sorted(glob.glob(f"{dir}*"))
    if len(d):
        n = max(int(x[len(dir) : x.find("_") if "_" in x else None]) for x in d) + 1
    return dir + str(n) + (f"_{comment}" if comment else "")
...
def plot_wh_methods():  # from vitg.utils.utils import *; plot_wh_methods()
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                ...
                if gt or conf[j] > 0.3:  # 0.3 conf thresh
                    label = f"{cls}" if gt else "%s %.1f" % (cls, conf[j])
                    plot_one_box(
...
def plot_test_txt():  # from vitg.utils.utils import *; plot_test()
...
def plot_targets_txt():  # from vitg.utils.utils import *; plot_targets_txt()
...
def plot_study_txt(
    f="study.txt", x=None
):  # from vitg.utils.utils import *; plot_study_txt()
    # Plot study.txt generated by test.py
    fig, ax = plt.subplots(2, 4, figsize=(10, 6), tight_layout=True)
    ax = ax.ravel()

    fig2, ax2 = plt.subplots(1, 1, figsize=(8, 4), tight_layout=True)
    for f in [f"coco_study/study_coco_yolov4{x}.txt" for x in ["s", "m", "l", "x"]]:
...
def plot_evolution(
    yaml_file="runs/evolve/hyp_evolved.yaml",
):  # from vitg.utils.utils import *; plot_evolution()
...
def plot_results_overlay(
    start=0, stop=0
):  # from vitg.utils.utils import *; plot_results_overlay()
...
):  # from vitg.utils.utils import *; plot_results()
